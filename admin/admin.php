...
function UsunPodstrone($id, $connection)
{
    $id_clear = htmlspecialchars($id);
    ...
    if ($delete = mysqli_query($connection, $query_delete)) {
        echo '
...
function ListaKategorie($connection)
{
    if (isset($_SESSION)) {

        $queryMother = "SELECT * FROM categories WHERE category_mother IS NULL LIMIT 100";
        $resultMother = mysqli_query($connection, $queryMother);
        ...
        while ($rowMother = mysqli_fetch_array($resultMother)) {
            echo '
                    ...
                    <td><a class="edit" href="shop_panel.php?id_edit_category=' . $rowMother['id'] . '">Edytuj</a></td>
            ...
            $resultSubCategories = mysqli_query($connection, $querySubCategories);
            echo '<tr><td></td>';
            echo '<td colspan="3">';
            echo '<table class="category-list">';
            while ($rowSubCategories = mysqli_fetch_array($resultSubCategories)) {
                echo '
                    ...
                    <td><a class="edit" href="shop_panel.php?id_edit_category=' . $rowSubCategories['id'] . '">Edytuj</a></td>
...
function ListaProdukty($connection)
{
    if (isset($_SESSION)) {
        ...
        while ($row = mysqli_fetch_array($result)) {
            echo '
                    ...
                    <td><a class="edit" href="product_panel.php?id_edit_product=' . $row['id'] . '">Edytuj</a></td>
...
function UsunKategorie($id, $connection)
{
    $id_clear = htmlspecialchars($id);

    $query = "SELECT * FROM categories WHERE id = $id LIMIT 1";
    $category_info_query = mysqli_query($connection, $query);
...
if (isset($_GET['id_edit'])) {
    EdytujPodstrone($_GET['id_edit'], $connection);
}
if (isset($_GET['id_delete'])) {
    UsunPodstrone($_GET['id_delete'], $connection);
}
if (isset($_GET['id_edit_category'])) {
    EdytujKategorie($_GET['id_edit_category'], $connection);
}
if (isset($_GET['id_delete_category'])) {
    UsunKategorie($_GET['id_delete_category'], $connection);
}
if (isset($_GET['id_edit_product'])) {
    EdytujProdukt($_GET['id_edit_product'], $connection);
}
if (isset($_GET['id_delete_product'])) {
    UsunProdukt($_GET['id_delete_product'], $connection);
}
?>
</body>
